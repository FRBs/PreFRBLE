...
from Rays import CreateChoppedRaySegments, CreateLoSsDMRM
from LikelihoodFunctions import MakeDMRMRayLikelihoodFunction
...
span_tot = 8   ## number of LoS to be computed
...
## combine segments to full LoS and reduce raw data to LoS observables, results saved to DMRMrays_file
#CreateLoSsDMRM( models=models )

### repeat the steps above until you reach a decent amount of LoS reduced to observables in DMRMrays_file
...
P = MakeDMRMRayLikelihoodFunction( bins, DM_range, measure='DM', absolute=False, bunch=bunch, model='primordial' )
for model in models:
    P = MakeDMRMRayLikelihoodFunction( bins, RM_range, measure='RM', absolute=True, bunch=bunch, model=model )
